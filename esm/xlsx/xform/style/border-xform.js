...
class EdgeXform extends BaseXform {
  ...
  render(xmlStream, model, defaultColor) {
    ...
    if (model && model.style) {
      xmlStream.addA('style', model.style)
      if (color) {
...
class BorderXform extends BaseXform {
  ...
  render(xmlStream, model) {
    ...
    if (model.diagonal && model.diagonal.style) {
      if (model.diagonal.up) {
        xmlStream.addA('diagonalUp', '1')
      }
      if (model.diagonal.down) {
        xmlStream.addA('diagonalDown', '1')
      }
    }
  ...
  parseOpen(node) {
    ...
    switch (node.name) {
      case 'border':
        this.reset()
        this.diagonalUp = parseBoolean(node.atts.diagonalUp)
        this.diagonalDown = parseBoolean(node.atts.diagonalDown)
        return true
      default:
