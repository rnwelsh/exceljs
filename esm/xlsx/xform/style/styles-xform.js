...
export default class StylesXform extends BaseXform {
  ...
  render(xmlStream, model) {
    ...
    xmlStream.oN('styleSheet', StylesXform.STYLESHEET_ATTRIBUTES)
    if (this.index) {
      // model has been built by style manager role (contains xml)
      if (model.numFmts && model.numFmts.length) {
        xmlStream.oN('numFmts', { count: model.numFmts.length })
      ...
      xmlStream.oN('fonts', { count: model.fonts.length, 'x14ac:knownFonts': 1 })
      model.fonts.forEach(fontXml => {
        xmlStream.writeXml(fontXml)
      })
      xmlStream.cN()
      xmlStream.oN('fills', { count: model.fills.length })
      model.fills.forEach(fillXml => {
        xmlStream.writeXml(fillXml)
      })
      xmlStream.cN()
      xmlStream.oN('borders', { count: model.borders.length })
      model.borders.forEach(borderXml => {
        xmlStream.writeXml(borderXml)
      })
      xmlStream.cN()
      this.map.cellStyleXfs.render(xmlStream, [{ numFmtId: 0, fontId: 0, fillId: 0, borderId: 0, xfId: 0 }])
      xmlStream.oN('cellXfs', { count: model.styles.length })
