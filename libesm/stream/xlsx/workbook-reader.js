...
import { setGracefulCleanup, file } from 'tmp'
...
class WorkbookReader extends EventEmitter {
  ...
  _getStream(input) {
    if (input instanceof _Readable || input instanceof Readable) {
      return input;
    }
          ...
          if (entry.path.match(/xl\/worksheets\/sheet\d+[.]xml/)) {
            ...
            if (this.sharedStrings && this.workbookRels) {
              ...
              await new Promise((resolve, reject) => {
                file((err, path, fd, tempFileCleanupCallback) => {
                  if (err) {
                    return reject(err);
                  }
                  waitingWorkSheets.push({sheetNo, path, tempFileCleanupCallback});

                  const tempStream = createWriteStream(path);
